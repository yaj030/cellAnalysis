...
class COUNTCELLDIVISION:
    def __init__(self, scope,folderName=None,positions = [],colors=['phase','GFP','iRFP'],
        color2use='iRFP',contrast=(1,99), rows=5,cols=20):

        namaPrefix = ''
        self.experiment = agingExperiment()
        self.scopeUsed = scope
        self.contrast = contrast
        self.color2useIdx = colors.index(color2use)+1
    ...
    def getSeparatedImages(self):
        self.separatedImage = self.analysis.experimentObj.output4Separated(\
            [self.currentPosition],color=self.color2useIdx,outputTif=False)
        self.separatedImagePhase = self.analysis.experimentObj.output4Separated(\
...
class TRACKMOTHER:
    ...
    def getImages(self):
        img = img_as_ubyte(equalize_hist(self.motherTrackerManager.separatedImagePhase[self.ch])*0.6)
        img2 = self.motherTrackerManager.separatedImage[self.ch]
        low, high = np.percentile(img2, self.motherTrackerManager.contrast)
        img2 = rescale_intensity(img2, in_range=(low, high),out_range=(0,255))
        ...
        self.all_images_color = np.copy(img)
        for fr in range(self.numb_frames):
            self.all_images_phase[:,:,fr] = img[:,:,fr]
            self.all_images_color[:,:,fr] = img2[:,:,fr]
        ...
        for ii in range(self.motherTrackerManager.numb_rows):
            ...
            for jj in range(self.motherTrackerManager.numb_cols):
                ...
                if ind<self.motherTrackerManager.numbFrames:
                    ...
                    tmp =self.all_images_color[:,:,ind]
                    #_, high = np.percentile(tmp,(1,99))
                    rowsA_color.append(tmp)
                else:
                    rowsA.append(np.zeros((self.height,self.width),dtype=np.uint8))
                    rowsA_color.append(np.zeros((self.height,self.width),dtype=np.uint8))
            #self.currentPageImgColor[idy:idy+self.height,:,:] = gray2rgb(np.hstack(tuple(rowsA)))
            phase = np.expand_dims(np.hstack(tuple(rowsA)),-1)
            color = np.expand_dims(np.hstack(tuple(rowsA_color)),-1)
            img = np.dstack((color,phase,phase))
            self.currentPageImgColor[idy:idy+self.height,:,:] = img
